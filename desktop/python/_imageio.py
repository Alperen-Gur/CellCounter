...
# A reader may already have materialized its native array, but these global
# budgets prevent the normalizer and image_prepare from making multi-gigabyte
# float/display/TIFF duplicates. They apply to every reader, not only VSI.
MAX_RAW_SOURCE_BYTES = 1024 * 1024 * 1024
MAX_PROJECTED_FLOAT_BYTES = 512 * 1024 * 1024


def _checked_product(values) -> int:
    result = 1
    for raw in values:
        value = int(raw)
        if value < 0:
            raise ImageTooLargeError("Invalid negative image dimension.")
        result *= value
        if result > (1 << 63) - 1:
            raise ImageTooLargeError(
                "Image dimensions overflow. Crop the acquisition or export a "
                "smaller OME-TIFF before importing.")
    return result


def _enforce_shape_budget(shape, axes: str, itemsize: int) -> None:
    """Reject unsafe arrays from dimensions alone; does not allocate pixels."""
    shape = tuple(int(value) for value in shape)
    axes = (axes or "").upper()
    if len(axes) != len(shape) or "Y" not in axes or "X" not in axes:
        axes = _infer_axes(shape)
    raw_bytes = _checked_product(shape) * max(1, int(itemsize))
    sizes = {axis: shape[index] for index, axis in enumerate(axes)}
    height = sizes.get("Y", shape[-2] if len(shape) >= 2 else 0)
    width = sizes.get("X", shape[-1] if shape else 0)
    channel_count = max(1, sizes.get("C", sizes.get("S", 1)))
    projected_bytes = _checked_product((height, width, channel_count)) * 4
    if raw_bytes > MAX_RAW_SOURCE_BYTES or projected_bytes > MAX_PROJECTED_FLOAT_BYTES:
        raise ImageTooLargeError(
            "This acquisition exceeds CellCounter's safe decode budget "
            "(1 GiB source / 512 MiB projected float image). Export a cropped "
            "region or smaller projected OME-TIFF, then import that file.")
...
def _to_hwc(arr, axes: str, z_project: str):
    ...
    if len(axes) != a.ndim or not axes:
        ...
        axes = inferred
    _enforce_shape_budget(a.shape, axes, a.dtype.itemsize)
...
def _read_vsi(path: str):
    """Olympus .vsi via OpenSlide (LGPL-2.1, dynamically loaded)."""
    import numpy as np
    import openslide
    ...
    try:
        ...
        if width <= 0 or height <= 0:
            raise ImageIOError(
                "image-open-failed", hint="OpenSlide reported invalid VSI dimensions")
        if width * height > 500_000_000:
            raise ImageTooLargeError(
                "VSI level 0 exceeds 500 megapixels; export the region of interest "
                "or a pyramidal OME-TIFF before analysis")
        image = slide.read_region((0, 0), 0, (width, height)).convert("RGB")
        arr = np.asarray(image)
        property_name = getattr(openslide, "PROPERTY_NAME_MPP_X", "openslide.mpp-x")
        px = _float_or_none(slide.properties.get(property_name))
    finally:
        slide.close()
    return arr, "YXS", {
        "channel_names": ["R", "G", "B"],
    ...
    if ext in VENDOR_PACKAGES:
        ...
        try:
            if ext == ".czi":
                ...
                arr, axes, extras = _read_vsi(path)
            else:
