...
Spawned by the Swift `TrainingService`. The contract this script emits on stdout:
    ...
    EPOCH {n} val_error={float} eta={int} lr={float}
...
def _autocast_ctx(device, enabled: bool = True):
    """Return a context manager — autocast on mps/cuda, no-op on cpu."""
    if not enabled or not _HAS_TORCH or device is None:
...
def _validation_error(model, val_images: list, val_masks: list, device,
                      mixed_precision: bool) -> float:
    """Measured validation error = 1 - Jaccard summary on held-out images."""
    if not val_images:
        return 0.0
    ...
    if not iou_scores:
        raise RuntimeError("validation inference failed for every held-out image")
...
def _final_eval(model, test_images: list, test_masks: list, device,
    ...
    if not preds_for_ap:
        raise RuntimeError("test inference failed for every held-out image")
    ...
    if not _HAS_CP_METRICS:
        raise RuntimeError("Cellpose average_precision metrics are unavailable")
    try:
        ...
        raise RuntimeError("Cellpose AP50 evaluation failed") from exc
...
def main() -> int:
    ...
    parser.add_argument("--early-stop", type=int, default=1, choices=[0, 1],
                        dest="early_stop")
    parser.add_argument("--mixed-precision", type=int, default=0, choices=[0, 1],
    ...
    if args.mixed_precision == 1 and not (
            device is not None and device.type in ("mps", "cuda")):
        print(json.dumps({"error": "mixed-precision-unavailable-on-cpu"}), flush=True)
        return 2
    ...
    if not pairs:
        print(json.dumps({"error": "no-image-mask-pairs"}), flush=True)
        return 2
    ...
    if not train_images or not val_images or not test_images:
        print(json.dumps({"error": "unloadable-or-incomplete-split",
                          ...
                          "test": len(test_images)}), flush=True)
        return 2
    ...
    if args.strict_schedule == 1:
        ...
        for epoch in range(1, total + 1):
            ...
            # Cellpose does not expose training loss from train_seg. Report only
            # the measured held-out validation error; never fabricate a proxy.
            val_error = _validation_error(
                model, val_images, val_masks, device, mixed_precision)
            elapsed = time.time() - start
            per = max(0.001, elapsed / epoch)
            eta = int(per * (total - epoch))

            _flush(f"EPOCH {epoch} val_error={val_error:.4f} "
            ...
            if epoch % 10 == 0:
                ...
                try:
                    if hasattr(model, "net") and hasattr(model.net, "save_model"):
                        model.net.save_model(str(ckpt_path))
                    elif _HAS_TORCH and hasattr(model, "net"):
        ...
        try:
            ...
            print(json.dumps({"error": "train-failed", "detail": str(exc)}),
                  flush=True)
            return 2
        _flush(f"STAGE trained_epochs={total} lr={args.lr:.6f}")

    # --- save final weights to --output ----------------------------------- #
    output_path = Path(args.output)
    try:
        if hasattr(model, "net") and hasattr(model.net, "save_model"):
            model.net.save_model(str(output_path))
        elif _HAS_TORCH and hasattr(model, "net"):
            torch.save(model.net.state_dict(), str(output_path))
        else:
            raise RuntimeError("Cellpose model exposes no supported checkpoint writer")
    except Exception as exc:
        _err(f"[cellpose_train] saving final weights failed: {exc!r}")
        return 2
    if not output_path.is_file() or output_path.stat().st_size < 1024:
        _err("[cellpose_train] final checkpoint is missing or empty")
        return 2
    ...
    if test_images:
        try:
            metrics = _final_eval(
                model, test_images, test_masks, device, mixed_precision)
        except Exception as exc:
            _err(f"[cellpose_train] final eval failed: {exc!r}")
            return 2
    else:
        ...
        try:
            ...
            _err(f"[cellpose_train] validation fallback eval failed: {exc!r}")
            return 2
