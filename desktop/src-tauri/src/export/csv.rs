...
fn build_summary_csv(
    ...
    let prov = Provenance {
        ...
        detector_runtime: "Per-image detector runtime is recorded in each summary row; this batch header records the configured family.".to_string(),
        checkpoint_identity: model_id.to_string(),
        weights_sha256: None,
        weights_sha256_reason: Some("A batch summary can contain images rerun with different checkpoints; use each image's provenance sidecar for a weights checksum.".to_string()),
        px_per_um,
