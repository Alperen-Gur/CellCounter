  ...
  return (
    <header className="topbar">
      ...
      <div className="top-actions">
        <span className="privacy-pill"><CloudOff size={14} /> Local only</span>
        <button className="icon-button" onClick={toggleTheme} aria-label={`Switch to ${theme === "light" ? "dark" : "light"} theme`}>
          {theme === "light" ? <Moon size={18} /> : <Sun size={18} />}
        </button>
        <button className="icon-button" onClick={onHelp} aria-label="Keyboard shortcuts"><CircleHelp size={18} /></button>
      </div>
    </header>
  );
}
