...
export function LibraryView({ images, activeId, onOpen, onRemove, onCondition, onImport, onImportFolder }: LibraryViewProps) {
  ...
  return (
    <main className="page-view library-view">
      <header className="page-heading"><div><span className="eyebrow">Local study</span><h1>Image library</h1><p>Saved in this browser on this device. Choose an image to analyze or review.</p></div><div className="toolbar-actions"><button className="secondary-button" onClick={onImportFolder}><FolderOpen size={15} /> Import folder</button><button className="run-button" onClick={onImport}>Import images</button></div></header>
      <div className="view-toolbar"><label className="search-field"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} aria-label="Find an image" placeholder="Find an image" /></label><span>{visible.length} of {images.length}</span></div>
      {visible.length ? (
        <div className="library-table" role="table" aria-label="Imported images">
          ...
          {visible.map((image) => (
            <div className={`table-row ${image.id === activeId ? "active" : ""}`} role="row" key={image.id}>
              <button className="image-cell" onClick={() => onOpen(image.id)}><img src={image.thumbnailUrl || image.objectUrl} alt="" /><span><strong>{image.fileName}</strong><small>{image.width} × {image.height} px · {image.analysisId ? "Processed" : "Not processed"}</small></span></button>
              ...
              <span className="mono-value">{image.calibrationSource === "default" || !image.calibrationSource ? "Unverified scale" : `${image.pxPerUm} px/µm`}</span>
