    ...
    var body: some View {
        ZStack {
            VStack(alignment: .leading, spacing: 0) {
                ...
                NavItemView(icon: "clock", label: "Processing", count: state.jobScheduler.activeCount > 0 ? state.jobScheduler.activeCount : nil,
                            isActive: state.view == .queue) { state.view = .queue }
